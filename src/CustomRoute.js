    ...
    fields.forEach((field) => {
      if (field.validation) {
        switch (field.type) {
          case "text":
            ...
            schema[field.name] = Yup.string()
              ...
              .min(
                ...
                `${field.label} must be more than ${minLength || 0} characters`
              )
              .max(
                maxLength !== undefined ? maxLength : Infinity,
                `${field.label} must not be more than ${
                  maxLength || "Infinity"
                } characters`
              );
            break;
          ...
          case "password":
            const { minimumLength, pattern, message } =
              field.passwordValidation || {};
            schema[field.name] = Yup.string()
              .required(`${field.label} is required`)
              .min(
                minimumLength,
                `${field.label} must be at least ${minimumLength} characters`
              )
